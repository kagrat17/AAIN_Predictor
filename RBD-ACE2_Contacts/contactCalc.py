...
cwd = os.getcwd()
f = open(cwd + "/Contact_Data/" + pdbFile + "_Contacts", mode="a")
parser = PDBParser(PERMISSIVE=True, QUIET=True)
data = parser.get_structure(pdbFile, cwd + "/../PDB_Files/" + pdbFile + ".pdb")
    ...
    for j in range(len(cAlphaA[0])):
        if(cAlphaE[1][i] - cAlphaA[1][j]) <= 7:
            ...
            contacts += cAlphaA[0][j] + " " + \
                str(int((cAlphaE[1][i] - cAlphaA[1][j])*100)/100) + "\n"
...
f.write("Total Contacts: " + str(countTot), end="\n")
f.write("Cutoff Distance: 7 Angstroms", end="\n")
f.write("\n")
f.write(str(len(data[0])) + " RBD Contact Residues:", end="\n")
for i in range(len(data[0])):
    f.write(str(data[0][i]) + "(" + str(data[1][i]) + ")", end="\n")
f.write("\n")
f.write(str(len(aceRes)) + " ACE2 Contact Residues:", end="\n")
for c in aceRes:
    f.write(str(c.split(" ")[0]) + "(" + str(aceRes[c]) + ")", end="\n")
f.write("\n")
f.write("\n")
